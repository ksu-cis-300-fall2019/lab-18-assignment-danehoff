...
namespace Ksu.Cis300.NameLookup
{
    ...
    public class Dictionary<TKey, TValue> where TKey : IComparable<TKey>
    {
        ...
        private static BinaryTreeNode<KeyValuePair<TKey, TValue>> RemoveMininumKey(BinaryTreeNode<KeyValuePair<TKey, TValue>> t, out KeyValuePair<TKey, TValue> min)
        {
            ...
            if(t.LeftChild != null)
            {
                BinaryTreeNode<KeyValuePair<TKey, TValue>> rMin = RemoveMininumKey(t.LeftChild, out min);
                return new BinaryTreeNode<KeyValuePair<TKey, TValue>>(t.Data, rMin, t.RightChild);
        ...
        private static BinaryTreeNode<KeyValuePair<TKey, TValue>> Remove(TKey key, BinaryTreeNode<KeyValuePair<TKey, TValue>> t, out bool removed)
        {
            ...
            int keyTemp = key.CompareTo(t.Data.Key);
            if (keyTemp > 0)
            {

                BinaryTreeNode<KeyValuePair<TKey, TValue>> rightUpdate = Remove(key, t.RightChild, out removed);
                BinaryTreeNode<KeyValuePair<TKey, TValue>> finalR = new BinaryTreeNode<KeyValuePair<TKey, TValue>>(t.Data, t.LeftChild, rightUpdate);
                return finalR;
            ...
            else if (keyTemp < 0)
            {
                ...
                BinaryTreeNode<KeyValuePair<TKey, TValue>> finalL = new BinaryTreeNode<KeyValuePair<TKey, TValue>>(t.Data, leftUpdate, t.RightChild);
                return finalL;
            ...
            else
            {
                ...
                else
                {
                    ...
                    BinaryTreeNode<KeyValuePair<TKey, TValue>> upRight = RemoveMininumKey(t.RightChild, out min);
                    BinaryTreeNode<KeyValuePair<TKey, TValue>> final = new BinaryTreeNode<KeyValuePair<TKey, TValue>>(min, t.LeftChild,upRight);
                    return final;
        ...
        public bool Remove(TKey k)
        {
            ...
            bool rv;
            _elements = Remove(k, _elements, out rv);
            return rv;
